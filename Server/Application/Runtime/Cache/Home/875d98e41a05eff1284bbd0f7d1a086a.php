...
<table class="table datatable" id="datatable">
    ...
    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
            ...
            <td>
                <?php if(empty($vo["rate"])): ?><div class="rate" id="<?php echo ($vo["id"]); ?>" data-score="0"></div>
                    <?php else: ?>
                    <div class="rate" id="<?php echo ($vo["id"]); ?>" data-score="<?php echo ($vo["rate"]); ?>"></div><?php endif; ?>
            </td>
        </tr><?php endforeach; endif; else: echo "" ;endif; ?>
    </tbody>
</table>
<?php if($israte == 0): ?><button type="button" class="btn btn-success" id="rate-btn" style="width: 5%;">提&nbsp;&nbsp;交</button>
    <?php else: ?>
...
<script>
    $(document).ready(function() {
        ...
        $('#rate-btn').on('click', function() {
            ...
            $.ajax({
                url: '<?php echo U("/Home/Index/a_rate_submit");?>',
                method: 'POST',
                data: data
        ...
        $('#rate-change-btn').on('click', function() {
            ...
            $.ajax({
                url: '<?php echo U("/Home/Index/a_rate_change");?>',
                method: 'POST',
                data: data
            }).success(function(dataget) {
                if(dataget == 1) {
